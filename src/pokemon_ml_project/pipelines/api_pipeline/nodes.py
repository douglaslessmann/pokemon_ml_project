...
class MLPredictor:
    def __init__(self, classifier_model, label_encoder):
    ...
    def predict(self, info_API: dict):
        ...
        return {"prediction": class_prediction_str[0]}


def save_predictor(classifier_model, label_encoder, pokemon_database):
    predictor = MLPredictor(classifier_model, label_encoder)
...
def teste_predict_function(predictor, pokemon_database):
    ...
    if np.isin(results["prediction"], pokemon_types):
        return True
    return False
